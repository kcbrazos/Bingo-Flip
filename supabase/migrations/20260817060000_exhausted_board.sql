...
--
-- Under lockout the squares are finite, and nothing said what happens when the last one is taken
-- and nobody has met the win condition. claim_square() ended a match on a win and on nothing else,
-- so the room simply stayed open with no legal move left in it: every square owned, no bingo, no
-- target reached, and the only way out was the host binning the match - which records no result.
--
-- It is not an edge case. A 5x5 split between two teams does not always contain a bingo, and under
-- points a target nobody reaches is one setting away.
--
-- THE RULE: the board decides it on the score, by the room's own scoring. Highest total takes the
-- match; a tie at the top is a genuine draw and the match finishes with no winner. Nothing here
-- invents a new quantity - it is the same squares-plus-bonus-per-line that claim_square's points
-- branch and scoreFor() already compute, which is what keeps the three of them in agreement.
--
-- Lockout only. Under non-lockout nobody can be denied a square, so "the board is full" would mean
-- every team holding all 25 - by which point they have long since met any reachable condition.
